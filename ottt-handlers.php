...
function ottt_disable_inactive_form_handler() {
    ...
    $sqlSelInactive = "SELECT * FROM `$customerTable` WHERE `ottt_customer_last_viewed` < $minLastViewed AND `ottt_customer_success` = 1 AND `ottt_customer_disabled` IS NULL GROUP BY `ottt_customer_email` ORDER BY `ottt_customer_last_viewed` ASC LIMIT 15;";
